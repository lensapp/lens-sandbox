...
`ENV` under that name is dropped, the variable holds the placeholder instead, and
the run names the connector that fills it and the source it displaced. Clear that
run's grant with `lns connector forget <ID> --run <RUN>` and start again, or drop
the variable.
