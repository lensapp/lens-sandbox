...
| `push` | `lns push` | Publishes the document as one OCI artifact at `<REF>`. Each `spec.filesets` `path` directory is packed into a layer of that same artifact, so the files and the declaration that mounts them share one digest; a `README.md` beside the document is packed into a `text/markdown` layer (`sandbox-spec.md` §7.2); each fuzzy `spec.tools` version is resolved and published as an exact pin. For a `spec.mixins` entry that names a local path, the document it names publishes first as its own artifact, beside `<REF>` and under the mixin's own `name`, tagged with its own digest, and the entry is pinned to that digest — the command lists those mixins and asks before it uploads anything, which `--yes` accepts without prompting. `--dry-run` does everything except the upload, prints the digests that would publish for every artifact, and says when a declared tool means a real digest may differ. When `spec.image` names a Containerfile ([sandbox-spec §3.1.1](sandbox-spec.md#311-image)), the push builds it where the build cache is cold, publishes the image into `<REF>`'s own repository — one grant covers both — and publishes a document whose `image` is that image's digest and whose `imageSource` is the path the author wrote; the Containerfile and its context go with it as one more layer ([sandbox-spec §7.3](sandbox-spec.md#73-the-build-source-layer)). A build runs on the architecture of the host that pushes, so the image publishes as one entry of an image index and `image` names that index: a push from a second architecture adds its entry beside the first, and a push whose architecture the index already holds with the same digest publishes nothing and says so ([sandbox-spec §6.2](sandbox-spec.md#62-one-image-index-per-published-document)). A push never publishes a document whose image it does not have. `--dry-run` there prints the build key, which architectures the index already holds and which one this push would add, and, only where this machine already answers that key, the digest it answers with; where it does not, it says the digest can be known only by building. `--rebuild` ignores the build cache for this push. A push answers with data, so it takes `--format` ([§4.3](#43-machine-output)): `json` emits one object holding the reference, the artifact digest, the mixins it published, the README layer, the pinned tools, and `image` — `null` where `spec.image` is a reference, and otherwise the build key, this architecture's digest and the index the published document names, with a null digest where a dry run cannot know it. That object is the whole answer, so a `json` push never prompts: a document that publishes local mixins is refused without `--yes`. A built image over `push.imageLimit` ([§3.5](#35-lns-config)) fails the push, naming the layer that grew it and the instruction that wrote it. |
| `pull` | `lns pull` | Shows you what the reference resolves to, then fetches it and its base image into the local store — including one lns built and published beside the artifact, which is a reference like any other by the time a consumer sees it. Where that reference names an image index, the entry for this host's architecture is what lands; an index holding none is refused before anything boots, naming the architectures it does hold and the `lns push` that would add this one ([sandbox-spec §6.2](sandbox-spec.md#62-one-image-index-per-published-document)). A sandbox that declares tools asks before running their installers in a disposable provisioning guest; `--yes` accepts that without prompting. The fetch is bound to the digest you were shown, so a tag that moves in between is refused. |
...
| `inspect` | `lns inspect` | Renders one artifact's resolved content, offline for a local path. An artifact published from a Containerfile prints its `image` line as the file it was built from and one digest per architecture the published index holds — `image: built from ./image/Containerfile (4 lines, context 2 files), arm64 sha256:…, amd64 sha256:…` — an architecture the host Docker daemon built rather than a build guest is marked `built outside the gate by the host Docker daemon` ([sandbox-spec §6.2](sandbox-spec.md#62-one-image-index-per-published-document)) — and also prints that file and lists the context it shipped with each file's size, read off the artifact's own build source layer ([sandbox-spec §7.3](sandbox-spec.md#73-the-build-source-layer)); nothing is built and nothing is run. `--mixin <REF>` resolves that mixin in first (repeatable), so you can preview a composition without running it. Against a local path the flag takes a local path too: the mixin it names is read from this machine and merged by the [`sandbox-spec.md` §3.3.2](./sandbox-spec.md#332-merge-rules) rules — after the document, in flag order — so the render stays offline and resolves nothing. A published reference there is refused, because an offline render has nothing to resolve it against; the message names the two ways out, a local path or a published target. A mixin the document itself declares merges the same way, when it names a local path; one the document declares by published reference is listed and not merged, and the line says so, because refusing the render over it would refuse every document written to publish. With no operand it renders `./lns.yaml`. |
...
| `push.imageLimit` | | What a built image may weigh before `lns push` refuses it, in MiB. Unset means 4 GiB. |
| `build.engine` | | Which engine builds a Containerfile `spec.image` names: `lns`, the build guest, which is the default; or `docker`, the host Docker daemon over the Docker Engine API ([sandbox-spec §3.1.1](sandbox-spec.md#311-image)). The gate does not apply to a `docker` build, and `lns inspect` and the run summary say so. Any other value is refused, naming the two. |
| `build.dockerSocket` | | The Unix socket the `docker` engine is reached at. Unset means `$DOCKER_HOST` where it names a `unix://` socket, else `/var/run/docker.sock`. |
