...
| `list`                | List what is installed: what each connector serves, each method and what it still needs (`ready to grant`, `connect first`, or that this `lns` cannot offer it), and the profiles this machine holds. |
| `connect <ID>`        | Ask for the value each of the method's credentials needs, at the terminal and without echoing it, and keep the result as a named connection. `--as` names it; otherwise `lns` suggests the first free name and you confirm, so a suggested name never replaces a profile you already hold. A name you give yourself — with `--as` or at the prompt — that a profile already uses re-authenticates that profile in place. Grants nothing: a project still decides whether to use it. Refused for a method with no `auth` — grant that instead. |
...
profile to use or connect a new one. Answering it applies the method to the
