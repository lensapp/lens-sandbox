...
| `grant <ID>`          | Let one run use one method. `--run` names it — its id, its name, or a unique id prefix — and is required. Prints what that method opens, writes and sets, and the authority of each connection held, then asks. Where `--run` names no run, the disclosure says so and your answer reserves the decision for the run you next create with that name. An id or id prefix that resolves to nothing is an error, not a reservation. Exits `1` when you decline, and `1` when that run already granted that method and connection. |
