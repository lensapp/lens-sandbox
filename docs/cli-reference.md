...
lns artifact ls [--kind <image|sandbox|mixin>] [--format <table|json>]
...
| `tag`      | `lns tag`    | Re-reference a cached artifact under a new tag (`docker tag`-style). |
| `ls`       | —            | List what the local store holds — reference, kind, digest, size, and the sandbox holding each. `--kind` filters to `sandbox` (a pulled or built artifact), `mixin` (a pulled mixin document), or `image` (the base OCI image one runs on). Alias: `list`. |
