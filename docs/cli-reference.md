...
| `push`     | `lns push`     | Build `./lns.yaml` and upload it to a registry as a sandbox artifact, in one step. `<REF>` is the registry reference to publish at. Each `spec.filesets` `path` directory is packed into a FileSet artifact, pushed alongside, and pinned by digest in the published config; each fuzzy `spec.tools` version resolves against the tool's public version index and publishes as an exact pin. `--dry-run` validates, packs, and builds all of it offline, prints the digests that would publish, and uploads nothing (declared tools are not resolved — it notes when the published digest may differ). `-f`/`--file` publishes another definition file instead. |
| `pull`     | `lns pull`     | Inspect and fetch a published sandbox and its base image into the local cache. If it declares tools, disclose them and ask before running their installers in a disposable provisioning guest; `--yes` accepts them non-interactively. The pull is bound to the inspected digest. |
