...
spike_state=${OPENSHELL_TERMINAL_TEST_STATE:-$(mktemp -d /private/tmp/lns-os-terminal.XXXXXX)}
export LNS_HOME="$spike_state/home"
export LNS_SOCKET_PATH="$spike_state/lns.sock"
echo "Terminal E2E state: $spike_state"
trap '"$root/target/debug/lns" service stop' EXIT HUP INT TERM
"$root/target/debug/lns" service start
perl -e 'alarm 180; exec @ARGV or die $!' expect -f "$root/spikes/openshell-network/e2e/${1:-terminal.exp}" "$root"
