//! The conversions that have to match `parse-dockerfile`'s `#[non_exhaustive]`
//! enums. A variant upstream adds later is a case no test can construct, so the
//! wildcard arms that answer for one live here alone, away from the subset gate
//! they feed.
...
pub(super) fn command(arguments: &Upstream<'_>) -> super::Command {
    match arguments {
        Upstream::Exec(exec) => super::Command::Exec(super::strings(&exec.value)),
