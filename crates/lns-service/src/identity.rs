...
/// The `User-Agent` every request this service makes must carry, so a registry or connector sees `lns`, never the HTTP library underneath.
...
mod tests {
    ...
    #[test]
    fn every_header_names_this_service_version_and_the_traffic_it_belongs_to() {
