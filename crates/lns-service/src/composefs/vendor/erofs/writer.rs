    ...
    fn inode_meta(&self) -> (format::DataLayout, u32, u64, usize) {
        let (layout, u, size) = match &self.content {
            ...
            tree::LeafContent::Symlink(target) => {
                assert!(
                    target.len() <= crate::composefs::vendor::SYMLINK_MAX,
                    "symlink target is {} bytes (max {})",
                    target.len(),
                    crate::composefs::vendor::SYMLINK_MAX,
                );
...
impl<'a, ObjectID: FsVerityHashValue> InodeCollector<'a, ObjectID> {
    fn push_inode(&mut self, stat: &'a tree::Stat, content: InodeContent<'a, ObjectID>) -> usize {
        ...
        {
            // fs-verity is out of scope — descriptor-digest pinning
            // is the load-bearing anti-tamper mechanism. The kernel
            // treats a zero-length `trusted.overlay.metacopy` as
            // "metadata-only inode, skip integrity check"
            // (Documentation/filesystems/overlayfs.rst);
            // a populated metacopy (the upstream default) would make
            // overlayfs refuse the redirect unless the target file
            // carries a matching fs-verity descriptor, which our
            // content store doesn't ship.
            //
            // Keep the OverlayMetacopy import path live for the reader
            // module + for the day we enable fs-verity behind a flag;
            // emit `&[]` for now.
            let _ = OverlayMetacopy::new(id);
            xattrs.add(b"trusted.overlay.metacopy", &[]);
