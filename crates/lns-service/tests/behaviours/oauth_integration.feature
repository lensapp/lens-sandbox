Feature: lns-service oauth integration sign-in
  ...
  Scenario: First use of an unconnected oauth integration signs in, connects, and arms it
    Given an unconnected "some-oauth" oauth integration whose sign-in will complete
    ...
    Then the "some-oauth" integration is connected live
  ...
  Scenario: A device sign-in that expires fails the held request and stores nothing
    Given an unconnected "some-oauth" oauth integration whose sign-in will expire
    When a workload request carries the "some-oauth" placeholder
    ...
    And the "some-oauth" integration is not connected

  Scenario: A device sign-in denied in the browser stores nothing
    Given an unconnected "some-oauth" oauth integration whose sign-in will be denied
    When a workload request carries the "some-oauth" placeholder
    And the developer accepts the prompt
    Then the held request is failed at the boundary
    And no token set is stored for "some-oauth"
