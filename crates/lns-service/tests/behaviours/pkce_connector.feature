Feature: lns-service pkce connector sign-in
  Some connectors authenticate by a browser-redirect OAuth 2.0
  authorization-code flow with PKCE rather than a device code or a pasted
  secret. A pkce connector carries an `oauth` block with `flow: pkce` (an
  ...
  the existing connect/hold/notify pipeline: an un-connected pkce connector
  ...
  Scenario: First use of an unconnected pkce connector signs in via browser, connects, and arms it
    Given an unconnected "some-pkce" oauth connector using the pkce flow whose sign-in will complete
    ...
    And the "some-pkce" connector is connected live
  ...
  Scenario: A pkce sign-in the developer cancels fails the held request and stores nothing
    Given an unconnected "some-pkce" oauth connector using the pkce flow
    When a workload request carries the "some-pkce" placeholder
