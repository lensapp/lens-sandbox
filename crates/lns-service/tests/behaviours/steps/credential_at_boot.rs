...
fn definition_with_credential(env_var: &str, domain: &str) -> String {
    format!(
        r#"{{"apiVersion":"lns.run/v1","kind":"sandbox","name":"hermes","spec":{{"image":"ghcr.io/team/base:1","credentials":[{{"envVar":"{env_var}","placeholder":"lns-placeholder-{env_var}","injections":[{{"kind":"bearer_header","domain":"{domain}"}}]}}]}}}}"#
    )
}

#[given(
    regex = r#"^the sandbox definition declares a credential "([^"]+)" injected on "([^"]+)"$"#
)]
fn definition_declares_credential(w: &mut BehaviourWorld, env_var: String, domain: String) {
    let rig = w.declared.get_or_insert_with(Default::default);
    rig.definition = Some(definition_with_credential(&env_var, &domain));
}
