Feature: declared filesets ship files inside the sandbox artifact
  ...
  (path — packed into a layer of that same artifact by push) or a small
  inline UTF-8 file map carried by the sandbox definition itself. A
  fileset is not a separate artifact, so the files and the declaration
  that mounts them share one digest. At launch the files are
  ...
  Scenario: running a pulled sandbox shows its declared filesets in the summary
    Given a pulled sandbox whose view declares fileset path "./skills" mounted at "/root/.agent/skills"
