...
#[then(regex = r#"^getting "([^"]+)" prints "([^"]+)" and "([^"]+)" but not "([^"]+)"$"#)]
fn getting_prints_replaced(
    ...
    if lines.contains(&first.as_str())
        && lines.contains(&second.as_str())
        && !lines.contains(&gone.as_str())
    {
        Ok(())
    } else {
        Err(format!(
            "expected {first:?} and {second:?} without {gone:?}, got: {:?}",
            run.output
        ))
    }
}
