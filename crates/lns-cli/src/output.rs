...
/// Control characters escaped as `\xNN`, because an ESC written raw is a sequence the terminal
/// obeys rather than shows.
...
/// Widths count `char`s because the padding below does, and measuring in bytes pushed every row
/// holding a non-ASCII cell out of line.
...
"
    ...
    fn no_control_character_reaches_the_stream_from_a_cell() {
        let text = rendered(
            &["NAME", "STATE"],
            &[vec!["some-run\u{0}suffix".into(), "running".into()]],
        );
        assert!(
        ...
        assert!(
            text.contains("some-run\\x00suffix"),
