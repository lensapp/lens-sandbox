...
pub fn decide_host_paths(
    ...
    store: &dyn HostPathDecisionStore,
    assume_yes: bool,
    interactive: bool,
    input: &mut dyn BufRead,
    output: &mut dyn Write,
) -> Result<HostPathGrant> {
    let DocumentOrigin::Pulled { reference } = origin else {
        return Ok(HostPathGrant::default());
    };
    let declared: Vec<&FilesetSummary> = filesets.iter().filter(|f| f.from_host()).collect();
    ...
    for fileset in declared {
        let host_path = fileset.host_path.as_deref().unwrap_or_default();
        let key = decision_key(reference, host_path);
        let decision = match recorded.get(&key) {
            ...
            None => {
                let asked = ask(reference, fileset, interactive, input, output)?;
                recorded.insert(key, asked);
                save(store, &recorded)?;
                asked
            }
        };
        settled.push((fileset, decision));
    }
    let mut grant = HostPathGrant::default();
    for (fileset, decision) in settled {
        apply(decision, fileset, &mut grant)?;
    }
    Ok(grant)
}
...
fn save(store: &dyn HostPathDecisionStore, recorded: &HostPathDecisionFile) -> Result<()> {
    store
        .save(recorded)
