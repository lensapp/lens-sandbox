...
Feature: host access forwards the host's signing agent into the guest
  ...
  Scenario: a commit made in the guest carries a signature that verifies
    ...
    When the user runs a microVM command "/bin/sh -c 'git commit --allow-empty -S -m x && git log --show-signature -1'" with host access "git-signing"
  ...
  Scenario: the verified signature reports full trust, not unknown validity
    Given the Lens Sandbox service is running
    And the host agent holds the key named by user.signingkey
    When the user runs a microVM command "/bin/sh -c 'git commit --allow-empty -S -m x && echo trust=$(git log --format=%G? -1)'" with host access "git-signing"
    Then the exit code is 0
  ...
  Scenario: the guest holds the signing key as a stub with no private material
    Given the Lens Sandbox service is running
    And the host agent holds the key named by user.signingkey
    When the user runs a microVM command "/bin/sh -c 'gpg --batch --list-secret-keys'" with host access "git-signing"
    Then the exit code is 0
  ...
  Scenario: the private key cannot be exported through the forwarded agent
    Given the Lens Sandbox service is running
    And the host agent holds the key named by user.signingkey
    When the user runs a microVM command "/bin/sh -c 'gpg --batch --export-secret-keys > /dev/null; echo rc=$?'" with host access "git-signing"
    Then the output does not contain "rc=0"
  ...
  Scenario: two signs at once each get their own connection
    Given the Lens Sandbox service is running
    And the host agent holds the key named by user.signingkey
    When the user runs a microVM command "/bin/sh -c 'echo a > /tmp/a; echo b > /tmp/b; gpg --batch --yes --detach-sign /tmp/a & gpg --batch --yes --detach-sign /tmp/b; wait; gpg --verify /tmp/a.sig /tmp/a && gpg --verify /tmp/b.sig /tmp/b'" with host access "git-signing"
    Then the exit code is 0
