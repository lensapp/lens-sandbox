Feature: a definition's credential requirements gate the launch before boot
  The unknown-id refusal for a declared connector, and the required
  ...
  aborted with guidance. A bare `spec.connectors` id never gates here — it
  ...
  Scenario: a required credential slot with no bound value refuses the launch
    ...
    And the output contains "lns connector connect some-provider"
