Feature: LNS tray-resident background service
  ...
  @gui
  Scenario: The user can quit LNS from the tray
    Given the LNS tray icon is visible
    When I select "Quit LNS" from the tray menu
    Then the tray icon disappears
    And no LNS service remains running

  @gui
  Scenario: The user can stop LNS from the CLI
    Given the LNS service is running
  ...
  Scenario: Commands that need the service fail clearly when it is not running
    ...
    And the error message reads: "LNS is not running. Run `lns service start` to start it."
