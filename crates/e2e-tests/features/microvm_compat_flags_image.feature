...
Feature: Docker-compat run flags that need a real image
  These boot a pulled alpine image (published as a minimal sandbox to an
  in-process registry, since a plain image REF is refused) and reach the
  network (a public registry) on a cold cache; like all microVM image work they run
  only via `make e2e-microvm`, never in CI.
  They cover the flag behaviours that need an explicit run REF: a command
