...
crates/lns-service/src/vm/guest_addr/real.rs             reservation composition root: cache::root + RealHostFiles/RealNeighbors/RealClock/FileStore construction, the Lease Drop release and the env gate; the allocator itself (exclusions, atomicity, expiry, reuse, restart reconcile, conflict detection) is host-tested at 100% in guest_addr.rs, and the file store's own read/write is the std::fs leaf
