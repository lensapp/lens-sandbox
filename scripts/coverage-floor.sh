...
crates/lns-cli/src/update/real.rs                        run wiring (current_exe + RealServiceClient construction + platform::detect), NativeHost (reqwest download, tempfile/ditto extraction, app destination resolution, and embedded installer invocation), checked_command, and native_cli_directory (defaults subprocess) are unmeasured adapters; update ordering and CLI directory selection are host-tested at 100% in update/native.rs, installer rollback in clients/macos/scripts/install.test.sh, and failed-download preservation in the packaged-service smoke. A real signed release update remains a manual acceptance check.
crates/lns-cli/src/uninstall/real.rs                     RealUninstallService/RealLoginAgent/RealFs leaves (send_request over the service socket, disable_login_agent, tokio::fs remove, quit_application via the embedded JXA script) + build_plan/purge_targets_from_env composition roots (current_exe canonicalize + cache_root/data_root/config/socket/dotfile path resolution + env-override reads) + pub async fn run wiring; the orchestration and the purge-target dir-vs-file decision (purge_targets/push_owned, incl. the override anti-rm-rf guard) are host-tested at 100% in uninstall/mod.rs via fakes. Pinned end-to-end by clients/macos/Smoke/run.py uninstalling a running app copy while retaining its data.
...
crates/lns-service/src/connector/mechanism/real.rs       leaf adapters for what lns lends a component (RealHttp reqwest, RealExec std::process, RealEntropy rand, RealRecorder) plus the composition root that holds one engine, one session store and the clock for this machine; the decision they are wired to lives in mechanism/selection.rs at 100%, the bounds each call is held to in mechanism/host.rs via fakes, the runtime itself in mechanism/wasm.rs against committed components, and the refresh schedule this file only starts in connector/refresh.rs.
...
crates/lns-service/src/dashboard/desktop.rs                  egui render surface — panels/areas/modal/immediate-viewport need a live frame + pointer input. The in-crate headless egui_kittest harness is no longer the blocker: it drives the wheel and real clicks through the AccessKit tree (both lists scroll, each keeps its own place, the view switch closes the audit detail), so the way to 100% is widening that harness one behaviour at a time. The pure residue (value/label formatting, friendly/relative time bucketing, and which answers an approval entry's row offers) lives at 100% in dashboard/{filter,sandboxes,live,format,approvals}.rs; load/apply_theme/viewport_builder and now_unix_secs are thin wiring exercised by the audit_dashboard example and driven live from tray.rs.
