...
crates/lns-session-broker/src/trust/real.rs             the cfg(linux) seed_trust_store composition root plus the two std::fs leaves it and the per-session CA install run on (RealTrustFs, RealTrustStore); their probe, read, mkdir, mode-setting write and append are host-tested against a tempdir here, and both decisions — copy-only-when-absent and append-only-when-missing — are host-tested at 100% in trust.rs via FakeTrustFs and FakeStore. Pinned end-to-end by the live microVM smoke.
...
crates/lns-session-broker/src/revforward/real.rs         bind/accept/dial-host/splice vsock leaf, the mirror of forward/real.rs; `#[cfg(target_os = "linux")]` and only runs inside the microVM. parse_cmdline logic lives in revforward/mod.rs at 100%. Pinned end-to-end by the docker-sidecar path.
...
crates/lns-service/src/image/real.rs                     production wiring over oci_client::Client; pinned by fake-Registry tests in image/mod.rs, where the client config (transport + linux/host-arch resolver) and the login-probe reference it builds from are host-tested at 100%
...
crates/lns-service/src/sidecar/launch.rs                 sidecar guest boot composition root (ingest → runtime_layer → composefs descriptor → upperfs → volume_store → vm::boot → broker session) plus the `VmGuests` adapter that hands it to the supervision loop, the same shape as run/orchestrator.rs and exercised the same way. Everything decided rather than wired lives at 100% elsewhere: the declared-to-runtime mapping and per-run port numbering in sidecar/mod.rs, the start/hold/stop loop and the proxy-CA ask in sidecar/supervise.rs, the two bridges in sidecar/bridge.rs, and the readiness probe in sidecar/ready.rs.
