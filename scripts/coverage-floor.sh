...
crates/lns-service/src/vm/host_net/real.rs               RealNeighbors shells out to /usr/sbin/arp, which exists only on macOS, so its parse arm cannot run on the Linux gate host; the ARP-table parser and every lease/network decision it feeds are host-tested at 100% in host_net.rs, and RealHostFiles is tempdir-tested here
