...
crates/lns-service/src/dashboard/mod.rs                  egui render surface — panels/areas/modal/immediate-viewport need a live frame + pointer input (no headless egui harness in-crate). The pure residue (value/label formatting, friendly/relative time bucketing, command execution) lives at 100% in dashboard/{filter,sandboxes,live,format,state,credentials,snapshot,commands}.rs; apply_theme/viewport_builder, now_unix_secs, and the file-backed FileCommandStores adapter behind commands::CommandStores are thin wiring exercised by the audit_dashboard example and driven live from tray.rs.
...
crates/lns-service/src/dashboard/unified.rs               egui render surface for the dashboard — panels, resizable tables, modals, and pointer input need a live frame (no headless egui harness in-crate). Every non-render decision it draws from lives at 100% in dashboard/{state,credentials,snapshot,commands,format}.rs, including the reveal state machine (toggle_reveal/is_revealed/hide_expired_reveal) behind the masked value fields.
