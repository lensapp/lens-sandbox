...
crates/lns-service/src/connector_store/real.rs           composition root: builds the caching registry + loads the pulled catalog / credential state / clock and persists on success; the pull-and-gate logic (pull_connector_with, connector_pull_response, the change diff) is host-tested at 100% in connector_store/mod.rs via a fake Registry
...
crates/lns-cli/src/connector/real.rs                     RealConnectorSignIn + RealConnectorPublisher leaves: open the service UnixStream to write BeginConnectorSignIn and render the streamed device-flow frames, and push a built connector artifact via build::push::push_artifact; the connect orchestration (service-unavailable / failed / completed) and the publish orchestration (build / dry-run / clientSecret refusal / upload) are host-tested at 100% in connector/mod.rs via FakeSignIn and FakePublisher. Pinned end-to-end by manual `lns connector connect github_oauth` smoke.
