...
crates/lns-session-broker/src/network/real.rs           RealCommandRunner/RealFsWriter leaf (std::process::Command + std::fs + the exists probe) + the cfg(linux) set_up composition root; the static claim/announce/gateway sequence, the DHCP path, arg-building and error-mapping are host-tested at 100% in network.rs via FakeCommandRunner/FakeFsWriter. Pinned end-to-end by the live microVM smoke.
...
crates/lns-service/src/vm/host_net/real.rs               RealHostNetwork shells out to /sbin/ifconfig and RealNeighbors to /usr/sbin/arp, both of which exist only on macOS, so their arms cannot run on the Linux gate host; the ARP-table parser, the discovery retry and every lease/network decision they feed are host-tested at 100% in host_net.rs, and RealHostFiles is tempdir-tested here
crates/lns-service/src/vm/guest_addr/real.rs             addressing composition root: the process-wide allocator over RealHostFiles/RealNeighbors/RealClock, the vsock fd the control channel is wrapped from, the Lease/ConflictMonitor Drop releases, the spawned monitor task and the env gate; the allocator and the whole reserve/configure/narrow sequence (exclusions, atomicity, expiry, reuse, refusal, release, conflict detection) are host-tested at 100% in guest_addr.rs over an injected network source and an in-memory guest
