...
crates/lns-service/src/ipc/adapter.rs                    Unix-socket bind / accept / streaming-request driver (run_server, handle_connection, handle_run, handle_exec, handle_logs, handle_attach, write_error, bind_or_replace_stale, is_instance_alive). Exercised end-to-end by Layer 1 e2e-tests through the real lns-service socket; handle_request, pump_responses, forward_session_input, stop_run_with, session-input helpers, map_signal, and the exec validate_exec / build_session_params pure helpers are host-tested at 100% in ipc/mod.rs, and the logs/attach follower loop (stream_to) plus the tee (tee_frames) in run_log.rs.
