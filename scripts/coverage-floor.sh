...
crates/lns-cli/src/integration/real.rs                   RealIntegrationSignIn leaf: opens the service UnixStream, writes BeginIntegrationSignIn, and renders the streamed device-flow frames; the connect orchestration (service-unavailable / failed / completed) is host-tested at 100% in integration/mod.rs via FakeSignIn. Pinned end-to-end by manual `lns integration connect github_oauth` smoke.
