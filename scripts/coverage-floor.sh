...
crates/lns-service/src/dashboard/mod.rs                  egui render surface — panels/areas/modal/immediate-viewport need a live frame + pointer input (no headless egui harness in-crate). The pure residue lives at 100% in dashboard/{filter,sandboxes,live}.rs; load/apply_theme/viewport_builder are thin wiring exercised by the audit_dashboard example and driven live from tray.rs.
