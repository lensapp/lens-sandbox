...
crates/lns-service/src/vm/netdev/real.rs                 macOS-only composition root: device_pair + HostSources/HostUpstream + getifaddrs for the subnet-overlap warning + the NetDevice guard that holds the Vz socket and every task serving the link. Only compiles on macOS, where no coverage job runs. Backend and subnet selection, the overlap warning, the socket pair and its buffers, the frame pump, the ethernet/ARP link, the DHCP server, the DNS relay and its resolver list, the boundary policy, the per-guest limits and the whole stack are host-tested at 100% in netdev.rs and its submodules, including an integration test that drives the stack with smoltcp as the guest.
