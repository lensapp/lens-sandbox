...
crates/lns-cli/src/policy/real.rs                        RealPolicyRegistry leaf: opens the service UnixStream, round-trips a PolicyPush/PolicyPull frame, and maps the response; push/pull orchestration (encode/decode, digest reporting, errors) is host-tested at 100% in policy/mod.rs via a FakePolicyRegistry. Pinned end-to-end by the manual `lns policy push`/`pull` smoke.
