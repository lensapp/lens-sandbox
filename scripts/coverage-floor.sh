...
crates/lns-cli/src/registry/real.rs                      RealRegistryClient leaf: opens the service UnixStream, round-trips a PushArtifact/Pull frame, and maps the response; push/pull orchestration (family inference, file→blob, digest reporting, errors) is host-tested at 100% in registry/mod.rs via a FakeRegistryClient. Pinned end-to-end by the manual `lns push`/`pull` smoke.
