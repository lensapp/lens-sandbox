...
crates/lns-service/src/ipc/adapter.rs                    Unix-socket bind / accept / streaming-request driver (run_server, handle_connection, handle_run, serve_prepared_run, handle_exec, handle_logs, handle_attach, bind_or_replace_stale, bind_secure, set_socket_mode_0600, peer_is_trusted, service_euid, is_instance_alive); the peer_is_authorized uid predicate, the run-start ordering (start_run, which decides every refusal a run can be turned away for before serve_prepared_run registers it) and write_error are host-tested at 100% in ipc/mod.rs. Exercised end-to-end by Layer 1 e2e-tests through the real lns-service socket; handle_request, pump_responses, forward_session_input, stop_run_with, session-input helpers, map_signal, and the exec validate_exec / build_session_params pure helpers are host-tested at 100% in ipc/mod.rs, and the logs/attach follower loop (stream_to) plus the tee (tee_frames) in run_log.rs.
