...
crates/lns-cli/src/service/orchestrator.rs               UnixStream-coupled CLI tail (run_image, exec_image, kill, ls, drive_attached_session, drain_to_exit, run_stdin_pump, pump_with_detector, run_winsize_forwarder, send_one_shot, dispatch, require_running, real_client). Exercised by Layer 1 e2e-tests through the real lns binary. The pure helpers in this file (plan_feed/drain_pending stdin-pump mapping, render_run_log level routing, render_pre_phase_log/phrase_for_verb, drive_pre_phase/pre_phase_step) are host-tested at 100% in this file's own mod tests; the sibling cmd_start/stop/status, require_running_check, socket_path, find_service_binary, parse_signal_name, render_ls_table, count_digits, friendly_started live in service.rs.
...
crates/lns-service/src/log.rs                            99%+ — match-arm `Level::INFO =>` and `Level::ERROR =>` pattern lines; LLVM places counter at the first body statement, not the `=>` line. Drop when coverage-strip-ast gains visit_arm stripping.
