...
crates/lns-service/src/dashboard/unified.rs               egui render surface for the dashboard — panels, resizable tables, modals, and pointer input need a live frame (no headless egui harness in-crate). Every non-render decision it draws from lives at 100% in dashboard/{state,credentials,snapshot,format}.rs.
