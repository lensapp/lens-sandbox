...
crates/lns-service/src/containerfile/real.rs             slice-1 capture composition root: the hook env read, cache::root/run_dir path resolution, the manifest-cache lookup of the base image, the spawn_blocking ext4 read, and the built-reference file write; the build it drives (capture → layer tar → config/manifest → import) is host-tested at 100% in containerfile/mod.rs. Slice 3 replaces this file with the executor.
