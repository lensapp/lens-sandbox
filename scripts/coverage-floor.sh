...
crates/lns-service/src/connector/mechanism/real.rs       leaf adapters for what lns lends a component (RealHttp reqwest, RealExec std::process, RealEntropy rand, RealRecorder) plus the composition root that holds one engine, one session store and the clock for this machine; the decision they are wired to lives in mechanism/selection.rs at 100%, the bounds each call is held to in mechanism/host.rs via fakes, and the runtime itself in mechanism/wasm.rs against committed components.
