...
crates/lns-session-broker/src/trust/real.rs             the cfg(linux) seed_trust_store composition root; RealTrustFs's symlink probe, read, mkdir and mode-setting write are host-tested against a tempdir here, and the copy-only-when-absent decision is host-tested at 100% in trust.rs via FakeTrustFs. Pinned end-to-end by the live microVM smoke.
