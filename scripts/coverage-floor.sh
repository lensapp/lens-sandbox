...
crates/lns-service/src/vm/netdev/real.rs                 per-platform composition root: cache::root + ensure_gvproxy (macOS) / located_passt (Linux) + RealSpawner + the NetDevice guard whose Drop kills the backend; each half is one cfg-gated wiring of host-tested parts. Backend selection, the gvproxy and passt argvs, the socket layout, binary lookup + checksum verification, the guest MAC, the vfkit datagram handshake and the spawn/wait/reap sequencing are host-tested at 100% in netdev.rs via FakeSpawner and a stand-in backend listener.
