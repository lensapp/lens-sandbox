...
crates/lns-artifact/src/containerfile/upstream.rs        the three wildcard arms this crate owes parse-dockerfile's #[non_exhaustive] Instruction and Command enums: a variant upstream adds later is a case no test can construct. Every other arm is a one-line span or string delegation, and the subset gate they feed — every refusal, its line and its alternative — is host-tested at 100% in containerfile.rs
...
crates/lns-service/src/containerfile/ext4_upper.rs       read-only ext4 leaf over `ext4-view`: 97.7% host-tested against a volume this repo's own writer formats and marks unclean (the recovery-flag clear, the upper-vs-root path mapping, every file-type mapping, both open failures), but that writer creates an empty filesystem and nothing in-process can put a file, a symlink or a whiteout device into an ext4 image — so the successful read_file/read_link lines need a volume a guest wrote. The walk they feed, the change set, the layer tar, the config/manifest assembly and the import are host-tested at 100% in containerfile/{upper,tar_layer,image,import,mod}.rs via a fake upper tree. Pinned end-to-end by the @microvm containerfile-layer scenario.
crates/lns-service/src/containerfile/real.rs             the executor's composition root: cache::root/run_dir path resolution, the Containerfile read, the pull a FROM resolves through and the peek a plan reads it with, the boot of one build step, the manifest-cache lookups (the parent's, and the built image a push hands the uploader), the real build context over std::fs, and the pointer and built-reference writes. It also holds the two leaves slice 7 adds: RealDaemon, one tokio UnixStream write-then-read-to-EOF against the host Docker socket, and the DockerHost impl that delegates to the ports beside it and imports what the daemon exported. Everything those two feed — the request bytes, the chunked answer, the ping refusal that names the switch, the build, the export, the docker-save read, the OCI assembly, the context tar and the key a daemon build is remembered under — is host-tested at 100% in containerfile/docker.rs through the Daemon and DockerHost ports. The projection of a cached manifest into a pushable image — the OCI media-type default, the negative-size floor, one path per layer — is host-tested at 100% in containerfile/image.rs, and the peek itself in image/mod.rs via FakeRegistry. The loop it drives — instruction order, ARG/ENV scoping, which instruction makes a layer, the exclusion list, the refusals — is host-tested at 100% in containerfile/executor.rs through the BuildHost port, the arguments one step boots with in containerfile/step.rs, the context walk in containerfile/context.rs, the file it names in containerfile/locate.rs, and the commit in containerfile/{tar_layer,image,import,mod}.rs. Pinned end-to-end by the three @microvm containerfile scenarios.
...
crates/lns-service/src/artifact/real.rs                  Layer-1 registry/FS composition roots: resolve_for_run/plan_resolved/plan_local/inspect wire the caching registry + content store + real FS, pull_packed_layer builds a RealRegistry for a reference and materialize_filesets wires the real cache root + content store over it, RealSnapshotDir walks a real directory, the pure inspection projection (project_inspection + declared_view_ports) and the disclosure of what the image index holds (built_architectures_of, over the Registry port) are host-tested at 100% in artifact/inspect.rs, packed-layer reading/size validation and the layer pull itself (pull_packed_layer_with, whose StreamingRegistry tests pin the stream/cleanup/digest-pin ordering) in artifact/fileset.rs, and the planning/assembly/audit logic in artifact/{mod,resources,policy,audit}.rs
crates/lns-cli/src/build/push.rs                         oci_client registry push leaf (auth handshake + push_blob/push_manifest_raw, for an artifact and for a built image) + the thin registry_auth_for wrapper that loads the real JSON auth store; the only ordering it owns is the one a registry requires — every blob before the manifest that references it — and the decisions around it are host-tested at 100% elsewhere: the client config (transport + lns identity) in build/mod.rs, credential selection (select_auth) and the auth error-mapping/recipe logic in build/push_auth.rs, the tags a built image and its index publish under and the index assembled over them (lns_artifact::image_index) and the artifact it uploads (lns_artifact::build) in lns-artifact. It also reads the repository back — one architecture's manifest with its config, and the index the tag names — which is registry I/O of the same kind: what those reads decide (which entry the index gains, which it replaces, whether anything is published at all) is host-tested at 100% in artifact/distribute.rs through the Producer port, and the one rule those reads own — a 404 or a MANIFEST_UNKNOWN/NAME_UNKNOWN envelope holds nothing, every other failure stops the push — is names_nothing_yet in build/push_auth.rs, host-tested at 100% there
...
crates/lns-cli/src/approval/real.rs                      RealApprovalService leaf: `lns approval`'s clap decode plus a one-line delegation to service::real::send_request over the service UnixStream; the verbs, their rendering, and every refusal path are host-tested at 100% in approval/mod.rs via a fake service. The live terminal-to-guest path is parked at Layer 1 (specs/microvm/approval_terminal.feature), which needs a booted guest.
...
crates/lns-service/src/dashboard/mod.rs                  egui render surface — panels/areas/modal/immediate-viewport need a live frame + pointer input. The in-crate headless egui_kittest harness is no longer the blocker: it drives the wheel and real clicks through the AccessKit tree (both lists scroll, each keeps its own place, the view switch closes the audit detail), so the way to 100% is widening that harness one behaviour at a time. The pure residue (value/label formatting, friendly/relative time bucketing, and which answers an approval entry's row offers) lives at 100% in dashboard/{filter,sandboxes,live,format,approvals}.rs; load/apply_theme/viewport_builder and now_unix_secs are thin wiring exercised by the audit_dashboard example and driven live from tray.rs.
