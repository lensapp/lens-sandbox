...
crates/lns-service/src/vm/host_net/real.rs               RealNeighbors shells out to /usr/sbin/arp and learn_shared_network spawns against it, both of which exist only on macOS, so their arms cannot run on the Linux gate host; the ARP-table parser, the discovery order and every lease/network decision they feed are host-tested at 100% in host_net.rs, and RealHostFiles and RealNetworkMemory are tempdir-tested here
crates/lns-service/src/vm/guest_addr/real.rs             reservation composition root: the process-wide allocator over RealHostFiles/RealNeighbors/RealClock, the Lease/ConflictMonitor Drop releases, the spawned monitor task and the env gate; the allocator itself (exclusions, atomicity, expiry, reuse, confirmation, conflict detection) is host-tested at 100% in guest_addr.rs, and AddressSelection::confirm is host-tested over an injected allocator in vm/session_client.rs
