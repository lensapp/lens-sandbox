...
crates/lns-service/src/vm/session_client/real.rs         streaming session driver (run_session select/spawn/join concurrency) + run_session_on_fd raw-vsock-fd intake + capture_session_output one-shot exec capture (stats probe) + set_nonblocking; the frame-decode loop (read_server_frames), input→ClientFrame mapping, and the stats parsing (guest_stats.rs) are host-tested at 100%. Exercised end-to-end by the live microVM.
crates/lns-service/src/vm/cloud_hypervisor/process.rs    tokio::process Spawner/Child syscall leaf (spawn/wait/start_kill); only runs against the real cloud-hypervisor + virtiofsd binaries. The launch sequencing + lifecycle (virtiofsd→socket-wait→relay→CH→connector, kill-on-failure) is host-tested at 100% in orchestrate.rs via FakeSpawner/FakeChild.
...
crates/lns-policy/build.rs                               OAuth env-substitution codegen: scans connectors.yaml for ${VAR} refs and resolves them from the build env; the substitution logic is host-tested at 100% in env_subst.rs

# Production-wiring adapters — thin pass-throughs; if logic is added, write a unit test and drop the entry.
crates/lns-service/src/browser.rs                        process-spawn leaf (`open`/`xdg-open`); only caller is the tray's sign-in card (itself IGNORES'd GUI). Pinned by manual `lns connector connect github_oauth` smoke.
...
crates/lns-service/src/artifact/real.rs                  Layer-1 registry/FS composition roots + the fileset pull loop: peek_and_plan/plan_local/inspect wire the caching registry + content store + real FS, materialize_filesets/pull_fileset_layers stream real blobs (StreamingRegistry integration tests here pin the stream/cleanup/digest-pin ordering), RealSnapshotDir walks a real directory, and refuse_*/effective_machine_catalog read the real connector catalog + credential stores; the pure inspection projection (project_inspection + declared_view_ports) is host-tested at 100% in artifact/inspect.rs, fileset artifact/size validation in artifact/fileset.rs, and the planning/assembly/audit logic in artifact/{mod,resources,policy,credential_boot,audit}.rs
crates/lns-cli/src/build/push.rs                         oci_client registry push leaf (auth handshake + push_blob/push_manifest_raw) + the thin registry_auth_for wrapper that loads the real JSON auth store; credential selection (select_auth) and the auth error-mapping/recipe logic are host-tested at 100% in build/push_auth.rs, and the artifact it uploads is built by lns_artifact::build (host-tested at 100%)
...
crates/lns-cli/src/connector/real.rs                     RealConnectorSignIn leaf: opens the service UnixStream, writes BeginConnectorSignIn, and renders the streamed device-flow frames; the connect orchestration (service-unavailable / failed / completed) is host-tested at 100% in connector/mod.rs via FakeSignIn. Pinned end-to-end by manual `lns connector connect github_oauth` smoke.
crates/lns-cli/src/login/real.rs                         RealRegistryVerifier leaf: opens the service UnixStream, writes RegistryLogin, decodes the one-shot verified/error reply, plus the run_login/run_logout composition roots (config default-registry read + socket wiring); login/logout/list orchestration is host-tested at 100% in login/mod.rs via FakeVerifier over a tempfile auth store. Pinned end-to-end by manual `lns login` smoke.
...
crates/lns-service/src/oauth/traits.rs                   DeviceFlow/Clock/AuthCodeFlow/CallbackListener port definitions + OauthConfig/PkceConfig From<OauthAuth>; trait/type-level declarations exercised via the fakes in oauth/mod.rs tests.
crates/lns-service/src/oauth/real.rs                     leaf adapters (RealDeviceFlow reqwest form POSTs for RFC 8628 device/poll/refresh; RealAuthCodeFlow pkce JSON exchange; RealCallbackListener loopback TcpListener; RealClock SystemTime); run_device_flow/run_pkce_flow/refresh_if_due orchestration host-tested at 100% in oauth/mod.rs via fakes. Pinned end-to-end by manual `lns connector connect github_oauth` / `openrouter` smoke.
...
crates/lns-service/src/ipc/adapter.rs                    Unix-socket bind / accept / streaming-request driver (run_server, handle_connection, handle_run, handle_exec, handle_logs, handle_attach, write_error, bind_or_replace_stale, bind_secure, set_socket_mode_0600, peer_is_trusted, service_euid, is_instance_alive); the peer_is_authorized uid predicate is host-tested at 100% in ipc/mod.rs. Exercised end-to-end by Layer 1 e2e-tests through the real lns-service socket; handle_request, pump_responses, forward_session_input, stop_run_with, session-input helpers, map_signal, and the exec validate_exec / build_session_params pure helpers are host-tested at 100% in ipc/mod.rs, and the logs/attach follower loop (stream_to) plus the tee (tee_frames) in run_log.rs.
crates/lns-cli/src/service/orchestrator.rs               UnixStream-coupled CLI tail (launch_run, run_image, exec_image, drive_attached_session, drain_to_exit, drain_after_chord, run_stdin_pump, pump_with_detector, run_winsize_forwarder, send_one_shot, dispatch, require_running, real_client). Exercised by Layer 1 e2e-tests through the real lns binary. The pure helpers in this file (plan_feed/drain_pending stdin-pump mapping, render_status_line/render_started_run/phrase_for_verb marker truth-table, needs_final_newline, render_attached_run_log debug-vs-status routing, drive_attached_session_with_writers over in-memory duplex writers, drive_pre_phase/pre_phase_step) are host-tested at 100% in this file's own mod tests; the sibling cmd_start/stop/status, require_running_check, socket_path, find_service_binary, parse_signal_name, friendly_started live in service.rs.

# Top-level GUI main-loop adapter — exercised live, not unit-tested.
crates/lns-service/src/tray.rs                           eframe main-loop adapter — `eframe::run_native` takes over the main thread (and on macOS needs a live NSApp), so the `eframe::App` impl and tray/menu wiring can only run live. Card rendering is no longer the blocker: the in-crate headless egui_kittest harness drives real clicks through the AccessKit tree (close ✕, pile pill, Deny scope mapping), and the pure residue (position_top_right arithmetic, prune_credential_inputs, visibility_transition, close_action/apply_dismissal, deny_request) is host-tested; load_icon decode is too. Getting to 100% needs the App impl extracting its per-frame logic behind a seam the harness can drive.
...
crates/lns-service/examples/credential_dashboard.rs      dev-only egui preview harness — seeds redacted synthetic fixtures and calls dashboard::render_credentials for live layout and interaction iteration; run manually via `cargo run -p lns-service --example credential_dashboard`, never run by `cargo test --all-targets`.
crates/lns-service/examples/dashboard.rs                 dev-only unified egui preview harness — seeds synthetic sandbox, credential, and audit state for live interaction iteration; run manually via `cargo run -p lns-service --example dashboard`, never run by `cargo test --all-targets`.
crates/lns-service/src/dashboard/credentials.rs          credential presentation models shared by the native dashboard surfaces; selectors and every semantic enum arm are unit-tested, while LLVM maps zero-count DAs to match/iterator/struct-literal opener lines in both production and test helpers.
crates/lns-service/src/dashboard/unified.rs              production unified egui surface — native panels, resizable tables, modals, pointer input, and timed secret reveal require a live frame; tray.rs supplies live data and executes service-owned commands while the dashboard example exercises preview operations manually.
