...
crates/lns-service/src/connector_store/real.rs           composition root: builds the caching registry + loads the pulled catalog / credential state / clock and persists on pull, and loads/removes/saves on remove; the pull-gate and remove-mapping logic (pull_connector_with, connector_pull_response, connector_remove_response, the change diff) is host-tested at 100% in connector_store/mod.rs via a fake Registry
...
crates/lns-cli/src/connector/real.rs                     RealConnectorSignIn + RealConnectorPublisher + RealConnectorPuller leaves: open the service UnixStream to drive sign-in, and push/pull/remove over IPC (encode Request, decode Response); the connect / publish / pull / remove orchestrations are host-tested at 100% in connector/mod.rs via FakeSignIn, FakePublisher, and FakePuller. Pinned end-to-end by manual `lns connector connect github_oauth` smoke.
