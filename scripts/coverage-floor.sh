...
crates/lns-service/src/containerfile/real.rs             the executor's composition root: cache::root/run_dir path resolution, the Containerfile read, the pull a FROM resolves through, the boot of one build step, the manifest-cache lookup of the parent, the real build context over std::fs, and the pointer and built-reference writes. The loop it drives — instruction order, ARG/ENV scoping, which instruction makes a layer, the exclusion list, the refusals — is host-tested at 100% in containerfile/executor.rs through the BuildHost port, the arguments one step boots with in containerfile/step.rs, the context walk in containerfile/context.rs, the file it names in containerfile/locate.rs, and the commit in containerfile/{tar_layer,image,import,mod}.rs. Pinned end-to-end by the three @microvm containerfile scenarios.
crates/lns-service/src/containerfile/parse/upstream.rs   the three wildcard arms this crate owes parse-dockerfile's #[non_exhaustive] Instruction and Command enums: a variant upstream adds later is a case no test can construct. Every other arm is a one-line span or string delegation, and the subset gate they feed is host-tested at 100% in containerfile/parse/mod.rs. Slice 2 of lensapp/lens-sandbox#393 moves this file to lns-artifact, which carries the same entry.
