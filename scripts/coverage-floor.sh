...
crates/lns-service/src/vm/netdev/real.rs                 macOS-only composition root: device_pair + /etc/resolv.conf read + SystemResolver + the NetDevice guard that holds the Vz socket and every task serving the link. Only compiles on macOS, where no coverage job runs. Backend selection, the socket pair and its buffers, the frame pump, the ethernet/ARP link, the DHCP server, the DNS gateway, the boundary policy and the whole stack are host-tested at 100% in netdev.rs and its submodules, including an integration test that drives the stack with smoltcp as the guest.
