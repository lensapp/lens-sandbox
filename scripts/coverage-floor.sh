...
crates/lns-service/src/artifact/real.rs                  Layer-1 registry/FS composition roots + the fileset pull loop: resolve_for_run/plan_resolved/plan_local/inspect wire the caching registry + content store + real FS, materialize_filesets/pull_packed_layer stream real blobs (StreamingRegistry integration tests here pin the stream/cleanup/digest-pin ordering), RealSnapshotDir walks a real directory, the pure inspection projection (project_inspection + declared_view_ports) is host-tested at 100% in artifact/inspect.rs, packed-layer reading/size validation in artifact/fileset.rs, and the planning/assembly/audit logic in artifact/{mod,resources,policy,audit}.rs
