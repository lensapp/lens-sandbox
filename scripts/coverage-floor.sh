...
crates/lns-cli/src/run/host_access/real.rs             production-wiring leaf (RealHostFacts over std::process::Command.output() and std::env::var); every locate/parse/projection decision is host-tested at 100% in run/host_access.rs via a scripted HostFacts, and the chain is pinned end-to-end by the @microvm host-access e2e
