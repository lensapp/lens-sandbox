...
crates/lns-session-broker/src/main.rs                    guest broker entry; static-musl PID-1 sibling, only runs inside the microVM. Exit-code clamping lives in exit.rs and the network refusal decisions live in network.rs, both host-tested at 100%.
