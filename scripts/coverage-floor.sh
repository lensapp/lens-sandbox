...
crates/lns-cli/src/build/push.rs                         oci_client registry push leaf (auth handshake + push_blob/push_manifest_raw, for an artifact and for a built image) + the thin registry_auth_for wrapper that loads the real JSON auth store; the only ordering it owns is the one a registry requires — every blob before the manifest that references it — and the decisions around it are host-tested at 100% elsewhere: the client config (transport + lns identity) in build/mod.rs, credential selection (select_auth) and the auth error-mapping/recipe logic in build/push_auth.rs, the tag a built image publishes under (lns_artifact::image::published_tag) and the artifact it uploads (lns_artifact::build) in lns-artifact
