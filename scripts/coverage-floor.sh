...
crates/lns-service/src/artifact/real.rs                  Layer-1 registry/FS adapter: peek_and_plan/inspect pull the real manifest+config, materialize_filesets/pull_fileset_layers pull real blobs, and RealSnapshotDir walks a real directory; the pure planning/assembly/audit logic is host-tested at 100% in artifact/{mod,fileset,resources,policy,credential_boot,audit}.rs
crates/lns-cli/src/build/push.rs                         oci_client registry push leaf (auth handshake + push_blob/push_manifest_raw) reusing the stored lns login credential; the artifact it uploads is built by lns_artifact::build (host-tested at 100%)
