...
crates/lns-cli/src/build/push.rs                         oci_client registry push leaf (auth handshake + push_blob/push_manifest_raw) reusing the stored lns login credential; the build/report logic it pushes is host-tested at 100% in build/report.rs and lns_artifact::build
crates/lns-cli/src/build/run.rs                          build-command I/O tail: reads the manifest file, writes the build cache, and dispatches check/build/push; the report/pin/pack logic is host-tested at 100% in build/report.rs, build/cache.rs, and build/collect.rs
crates/lns-cli/src/build/resolve.rs                      oci_client pull_manifest leaf resolving a bundle's floating component tags to digests; the pin/refs logic it feeds is host-tested at 100% in lns_artifact::build (pin_bundle_components/unpinned_component_refs)
crates/lns-cli/src/push.rs                               `lns push` I/O tail: reads the build cache off disk and pushes via oci_client; the cache plan/reconstruct logic is host-tested at 100% in build/cache.rs and the upload leaf in build/push.rs
...
crates/lns-cli/src/image/real.rs                         RealImageService leaf: one-line delegation to service::real::send_request over the service UnixStream; verb orchestration and rendering are host-tested at 100% in image/mod.rs via canned/fake services. Exercised end-to-end by Layer 1 e2e-tests' image cache management feature.
