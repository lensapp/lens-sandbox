...
crates/lns-policy/src/keychain/real.rs                   KeyringBlob leaf over keyring::Entry (run.lns/credentials) get/set_password + NoEntry mapping; store/selection orchestration host-tested at 100% in keychain/mod.rs via FakeBlob. Pinned end-to-end by manual real-keychain smoke.
crates/lns-cli/src/integration/real.rs                   RealIntegrationService leaf: opens the service UnixStream for the streamed sign-in frames and the one-shot RevokeIntegration; the connect/revoke orchestration (service-unavailable / failed / completed / cleared) is host-tested at 100% in integration/mod.rs via FakeSignIn. Pinned end-to-end by manual `lns integration connect github_oauth` smoke.
...
crates/lns-service/src/oauth/real.rs                     leaf adapters (RealDeviceFlow reqwest form POSTs for RFC 8628 device/poll/refresh; RealAuthCodeFlow pkce JSON exchange; RealCallbackListener loopback TcpListener; RealClock SystemTime); run_device_flow/run_pkce_flow/refresh_if_due orchestration host-tested at 100% in oauth/mod.rs via fakes. Pinned end-to-end by manual `lns integration connect github_oauth` / `openrouter` smoke.
