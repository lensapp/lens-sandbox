...
crates/lns-service/src/containerfile/ext4_upper.rs        read-only ext4 leaf over `ext4-view` (load_from_path + read_dir/read/read_link + the file-type mapping) against the ext4 a stopped guest wrote; the walk it feeds, the change set it produces, the layer tar, the config/manifest assembly and the import are host-tested at 100% in containerfile/{upper,tar_layer,image,import,mod}.rs via a fake upper tree. Pinned end-to-end by the @microvm containerfile-layer scenario.
