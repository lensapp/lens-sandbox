...
crates/lns-service/src/vm/netdev/real.rs                 macOS-only composition root: cache::root + ensure_gvproxy + RealSpawner + the NetDevice guard whose Drop kills the backend; only compiles on macOS, where no coverage job runs. Backend selection, the gvproxy argv, the socket layout, binary lookup + checksum verification, the vfkit datagram handshake and the spawn/wait/reap sequencing are host-tested at 100% in netdev.rs via FakeSpawner and a stand-in gvproxy listener.
