...
Monorepo. A Cargo workspace (production crates, two operator tools, two test/coverage infra crates) and one shell-script package; `Cargo.toml` `members` is the authoritative list. One crate sits deliberately outside it, because it builds for `wasm32-wasip2` and ships as committed `.wasm` bytes: the mechanism fixtures (`crates/lns-service/tests/fixtures/mechanism/`). `make lint` rebuilds them and fails if the committed bytes are stale.
