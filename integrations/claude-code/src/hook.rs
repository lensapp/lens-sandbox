...
pub fn process_hook(input: &str, exe: &str, auto_allow: bool) -> Option<Value> {
    ...
    match classify(command) {
        ...
        Decision::Sandbox(runtime) => {
            ...
            Some(hook_output(&rewritten, auto_allow))
        }
    }
}
...
mod tests {
    ...
    fn sandbox_worthy_command_is_rewritten() {
        let out = process_hook(&input_for("python app.py"), "/bin/lns-cc", true).unwrap();
    ...
    fn passthrough_command_returns_none() {
        assert!(process_hook(&input_for("ls -la"), "/bin/lns-cc", true).is_none());
    }

    #[test]
    fn invalid_json_returns_none() {
        assert!(process_hook("{not json", "/bin/lns-cc", true).is_none());
    }

    #[test]
    fn missing_command_field_returns_none() {
        let input = serde_json::json!({ "tool_input": {} }).to_string();
        assert!(process_hook(&input, "/bin/lns-cc", true).is_none());
    }
    ...
    fn auto_allow_disabled_omits_permission_decision() {
        let out = process_hook(&input_for("node x.js"), "/bin/lns-cc", false).unwrap();
    ...
    fn encode_helper_matches_embedded_payload() {
        let out = process_hook(&input_for("npx cowsay hi"), "/bin/lns-cc", true).unwrap();
        let rewritten = out["hookSpecificOutput"]["updatedInput"]["command"]
            .as_str()
            .unwrap();
